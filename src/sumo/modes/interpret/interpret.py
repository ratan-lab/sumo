...
class SumoInterpret(SumoMode):
    ...
    def run(self):
        ...
        # save classifier
        classifier_fname = "{}.pickle".format(self.output_prefix)
        pickle_handle = open(classifier_fname, "wb")
        pickle.dump({"model": model, "features": features.columns.values}, pickle_handle)
        pickle_handle.close()
        self.logger.info("#Sample classifier saved to {} file.".format(classifier_fname))
